document.addEventListener('DOMContentLoaded', () => {
        ...
        setTimeout(() => {
            fetch(url)
                ...
                .then(data => {
                    ...
                    const results = type === 'movies' ? data.results.sort((a, b) => a.episode_id - b.episode_id) : data.results;
    ...
    function loadCharacterDetails(characterUrl, moreInfoDiv) {
        fetch(characterUrl)
            ...
            .then(data => {
                moreInfoDiv.innerHTML = `
                    <p style="text-align:left"><strong>Mass:</strong> ${data.mass}</p>
                    <p style="text-align:left"><strong>Birth Year:</strong> ${data.birth_year}</p>
                    <p style="text-align:left"><strong>Height:</strong> ${data.height}</p>
                    <p style="text-align:left"><strong>Hair Color:</strong> ${data.hair_color}</p>
                    <p style="text-align:left"><strong>Skin Color:</strong> ${data.skin_color}</p>
                    <p style="text-align:left"><strong>Eye Color:</strong> ${data.eye_color}</p>
                    <p style="text-align:left"><strong>Gender:</strong> ${data.gender}</p>
                `;
            })
    ...
    loadData('https://swapi.dev/api/films/', 'movies', loadMovies);
    showTab('movies');
    ...
    charactersTab.addEventListener('click', () => {
        showTab('characters');
        loadData('https://swapi.dev/api/people/', 'characters', loadCharacter);
    });
});
